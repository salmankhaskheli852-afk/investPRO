...
const ConfirmPageComponent = () => {
    ...
    const handleSubmitDeposit = async () => {
        ...
        try {
            // Note: Global TID duplicate check is moved to Admin side for validation to avoid client-side permission issues.
            ...
            // 1. Set global transaction for Admin/Agent to review
            batch.set(globalTransactionRef, { ...transactionData, id: globalTransactionRef.id });

            // 2. Set user-specific transaction history
    ...
    return (
        <div className="bg-white -m-4 sm:-m-6 lg:-m-8 min-h-screen flex flex-col">
             <header className="sticky top-0 z-10 h-20 w-full bg-blue-900 text-white flex items-center justify-center text-center">
                <span className="font-bold text-2xl tracking-widest uppercase">investPro SAFE + FAST</span>
            </header>
            ...
            <main className="flex-1 p-4 space-y-5">
                ...
                <div className="p-4 bg-gray-50 rounded-lg space-y-3">
                    ...
                    {isLoadingChannel ? (
                        <p className='text-center py-4'>Loading account details...</p>
                    ) : selectedChannel ? (
                        <div className="space-y-2 text-lg">
                            ...
                            <div className="flex justify-between items-center border-b py-2">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <Copy className="h-5 w-5 text-gray-400 active:text-blue-500 cursor-pointer" onClick={() => handleCopy(selectedChannel.number, 'Account Number')} />
                                </div>
                            </div>
                             <div className="flex justify-between items-center border-b py-2">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <Copy className="h-5 w-5 text-gray-400 active:text-blue-500 cursor-pointer" onClick={() => handleCopy(selectedChannel.name, 'Account Name')} />
                                </div>
                            </div>
                            <div className="flex justify-between items-center border-b py-2">
                                <span className="font-medium text-gray-600">Quantity (مقدار)</span>
                                <div className="flex items-center gap-2">
                                    <span className="font-bold text-gray-900">{parseFloat(amount).toLocaleString()} PKR</span>
                                    <Copy className="h-5 w-5 text-gray-400 active:text-blue-500 cursor-pointer" onClick={() => handleCopy(amount, 'Amount')} />
                                </div>
                            </div>
                        </div>
                    ) : <p className='text-center text-destructive'>Could not load payment details. Please go back and try again.</p>}
                </div>
                ...
                <div className="p-4 bg-gray-50 rounded-lg space-y-3">
                    ...
                    <Input 
                        ...
                        className="h-12 text-base border-primary/50 focus:border-primary"
                    />
                </div>

                <div className="pt-4 pb-10">
                     <Button 
                        className="w-full h-14 text-lg rounded-lg bg-blue-900 hover:bg-blue-800" 
